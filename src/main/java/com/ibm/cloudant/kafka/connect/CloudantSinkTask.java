...
public class CloudantSinkTask extends SinkTask {
	...
	public void put(Collection<SinkRecord> sinkRecords) {
		...
		for (SinkRecord record : sinkRecords) {
			...
			if(jsonRecord.has(CloudantConst.CLOUDANT_DOC_ID)){
				jsonRecord.put(CloudantConst.SOURCE_DOC_ID, jsonRecord.get(CloudantConst.CLOUDANT_DOC_ID));
				if(guid_schema.toLowerCase().equals("kafka")) {
					jsonRecord.put(CloudantConst.CLOUDANT_DOC_ID, record.topic() + "_" + record.kafkaPartition().toString() + "_" + Long.toString(record.kafkaOffset()));	
				}
				else {
	...
	public void start(Map<String, String> props) {
 		...
 		try {
			...
			guid_schema = config.getString(InterfaceConst.GUID_SCHEMA);
